...
`src/features/dictionary/domain/compiled-package/` 是本契约的唯一权威。它的公开接口负责发布包事务、打开经校验的读取器、检查已同步的包状态，以及回答兼容性。manifest 类型、版本常量、规范化、路径规则、限制、源指纹与编译器契约都保持私有，只属于该模块及其 worker。
